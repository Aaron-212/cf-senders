...
const isInvalid = $derived(emailInvalid || (invalid ?? Boolean(errorText)));
const resolvedErrorText = $derived(errorText ?? (emailInvalid ? "Enter a valid email address." : undefined));
</script>
